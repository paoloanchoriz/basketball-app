	...
	module
			.directive(
					...
					[
							...
							function($parse, $compile, $timeout, baseOptions) {
								...
								return {
									...
									link : function(scope, elem, attrs,
										...
										if (!attrs.ngOptions) {
											throw new Error(
													'Expected ng-options attribute.');
										}

										var match = attrs.ngOptions
										...
										var displayFn = $parse(match[2]
												|| match[1]), valueName = match[4]
												|| match[6], groupByFn = $parse(match[3]
												|| ''), valueFn = $parse(match[2] ? match[1]
												: valueName), values = match[7], valuesFn = $parse(values);

										var options = getOptions(), searchModel = match[2] ? (match[2])
												.replace(new RegExp('^'
														+ valueName + '\\.'),
														'search.')
												: 'search', remoteSearch = typeof options.onSearch === 'function', timeoutHandle, lastSearch = '', focusedIndex = -1, getInitialSearchModel = function() {
											return match[2] ? {} : '';
										};
										...
										selectTemplate = '<select class="hide" ng-options="'
												+ attrs.ngOptions
												+ '" ng-model="'
												+ attrs.ngModel
												+ '" '
												+ (attrs.ngChange ? 'ng-change="'
														+ attrs.ngChange + '"'
														: '') + '></select>', dropdownTemplate = '<a class="dropdown-toggle" data-toggle="dropdown" href ng-class="{ disabled: disabled }">'
												...
												+ '<input class="'
												+ attrs.selectClass
												+ '" type="text" autocomplete="off" ng-model="'
												+ searchModel
												+ '" />'
												+ '</div>'
												+ '<ul role="menu">'
												...
												+ (remoteSearch ? ''
														: ' | filter: search')
												+ ').length" class="empty-result" stop-propagation="click">'
												+ '<em class="muted">'
												+ '<span ng-hide="'
												+ searchModel
												+ '">{{emptyListText}}</span>'
												+ '<span class="word-break" ng-show="'
												+ searchModel
												+ '">{{emptySearchResultText | format:'
												+ searchModel
												+ '}}</span>'
												+ '</em>'
												+ '</li>'
										...
										controller.$render = function() {
											// Added a timeout to allow for the
											// inner select element
											// to respond to the model change
										...
										// Watch for changes on the original
										// values and update the isolated child
										// scope accordingly
										...
										function setDisplayText() {
											var collection, locals = {}, text = undefined, key = selectElement
													.val();
										...
										function configChildScope() {
											...
											childScope.add = function() {
												options
														.onAdd(childScope.select);
											};

											if (remoteSearch) {
												inputElement.attr('ng-change',
														'onSearch('
																+ searchModel
																+ ')');
												childScope.onSearch = function(
														term) {
													if (timeoutHandle) {
